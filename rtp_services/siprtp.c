...
static void on_rx_rtp(void *user_data, void *pkt, pj_ssize_t size)
{
    ...
    status = pjmedia_rtp_decode_rtp(
					&strm->in_sess, pkt, (int)size, &hdr, &payload, &payload_len
				);
    if (status != PJ_SUCCESS) {
				...
				pj_status_t status)
{
    ...
    if (status != PJ_SUCCESS) {
		(THIS_FILE, "Error creating stream info from SDP", status);
		return;
    }
...
{
    unsigned i;
    puts("Listar todas las llamadas:");
    for (i=0; i<app.max_calls; ++i) {
	if (!app.call[i].inv)
	    continue;
	print_call(i);
...
int main(int argc, char *argv[])
{
    ...
    status = init_sip();
    if (status != PJ_SUCCESS) {
		app_perror(THIS_FILE, "Falló la inicialización", status);
		destroy_sip();
		return 1;
    }
    ...
    status = init_media();
    if (status != PJ_SUCCESS) {
	app_perror(THIS_FILE, "Falló la inicialización de la media", status);
	destroy_sip();
	return 1;
    }
	...
	PJ_LOG(3,(THIS_FILE, "Realizando %d llamada a %s..", app.max_calls,
			app.uri_to_call.ptr));

	for (i=0; i<app.max_calls; ++i) {
	    status = make_call(&app.uri_to_call);
	    if (status != PJ_SUCCESS) {
		app_perror(THIS_FILE, "Error al realizar la llamada", status);
		break;
	    }
	    pj_thread_sleep(app.call_gap);
	}
	...
	PJ_LOG(3,(THIS_FILE, "Listo para llamadas entrantes (max=%d)", 
			app.max_calls));
...
#if !PJ_HAS_THREADS
    PJ_LOG(3,(THIS_FILE, "Presiona Ctrl-C para salir"));
    for (;;) {
	pj_time_val t = {0, 10};
